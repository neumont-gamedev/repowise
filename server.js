...
http.createServer(async(req,res)=>{
 ...
 const paths={'/':'public/index.html','/app.js':'public/app.js','/style.css':'public/style.css','/analysis.js':'lib/analysis.js'};
 const path=paths[url.pathname];if(!path){res.writeHead(404);return res.end('Not found');}
 res.writeHead(200,{'Content-Type':path.endsWith('.html')?'text/html':path.endsWith('.css')?'text/css':'text/javascript','X-Content-Type-Options':'nosniff','Content-Security-Policy':"default-src 'self'; style-src 'self' 'unsafe-inline'; script-src 'self'; img-src 'self' data:; connect-src 'self'; frame-ancestors 'none'"});res.end(await readFile(path));
